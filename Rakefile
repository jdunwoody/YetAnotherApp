...
desc 'Regenerate storyboard identifiers in code. Only includes identifiers with prefix `YAA`.'
task :storyboard_ids do |t|
  `bundle exec sbconstants YAA/Constants/_FXStoryboardIdentifiers.h -s YAA/Resources/Storyboards/ -t Support/SBConstants-Templates/ --prefix=YAA`
end
...
desc 'Install Color palette'
task :install_colors do |t|
  colors_source = 'YAA/Resources/YAA-Colors.clr'
  colors_path = "#{Dir.home}/Library/Colors"
  mkdir_p colors_path
  cp colors_source, "#{colors_path}/YAA-Colors.clr"
end
