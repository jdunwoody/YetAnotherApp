...
SharedExamplesBegin(ModelSharedExamples)

    sharedExamplesFor(@"Models", ^(NSDictionary *data) {
        ...
        it(@"should implement a custom description method", ^{
            expect(model).to.respondTo(@selector(description));
        });
    });
...
SpecBegin(Weather)
    ...
    describe(@"Weather", ^{
        ...
        it(@"should Title case city name", ^{
            Weather *weather = [[Weather alloc] initWithCityName:@"my city"];

            expect(weather.cityName).to.equal(@"My City");
